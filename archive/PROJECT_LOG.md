...
- **[2026-09-10]** Data review part 2 (`docs/REVIEW-2026-09-10-part2.md`, "data collection 11"): 79 new log rows, trade #42 (BUY SL −$3.33, passes all gates incl. rejected PROX pair), true equity $422.35. Stall recovered as one 293.5-min hole (00:47→05:41, no trade open across it); drift fix survived the ~06:33 restart (`daily_losses: 2` correct). New to-dos: same-minute multi-rows (4 new, 11 all-time — INFO check in `check_data.py`, dedup guard in log path). 4-tool suite green (0 fail / PASSED). Strategy unchanged, no code changes.
